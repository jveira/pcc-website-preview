(() => {
  ...
  const toggleEditorKey = $('#toggle-editor-key');
  ...
  async function saveDraft() {
    try {
      ...
      const changedLabels = Object.keys(changed).map((path) => path.split('/').pop().replace(/\.json$/i, ''));
      const summary = changedLabels.join(', ');
      if (!window.confirm(`Publish these changes to the live site?\n\n${summary}\n\nThe site will update after its build finishes.`)) return;
      saveButton.disabled = true; saveButton.textContent = 'Publishing…';
  ...
  toggleEditorKey?.addEventListener('click', () => {
    const input = $('#editor-key');
    const visible = input.type === 'text';
    input.type = visible ? 'password' : 'text';
    toggleEditorKey.textContent = visible ? 'Show' : 'Hide';
    toggleEditorKey.setAttribute('aria-pressed', String(!visible));
    input.focus();
  });
  ...
  document.addEventListener('keydown', (event) => {
    if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === 's' && !event.shiftKey && !event.altKey) {
      ...
      if (!saveButton.disabled) saveDraft();
    }
  });
